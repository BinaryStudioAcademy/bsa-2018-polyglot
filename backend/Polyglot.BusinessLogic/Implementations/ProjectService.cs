...
namespace Polyglot.BusinessLogic.Implementations
{
    public class ProjectService : CRUDService, IProjectService
	{
		...
		{			
				...
				{
                DataAccess.NoSQL_Models.ComplexString temp = new DataAccess.NoSQL_Models.ComplexString() { Key = i.Key, OriginalValue = i.Value };

                // repository isn`t working now
                await stringsProvider.CreateAsync(new DataAccess.NoSQL_Models.ComplexString() { Key = i.Key, OriginalValue = i.Value });
            }			
