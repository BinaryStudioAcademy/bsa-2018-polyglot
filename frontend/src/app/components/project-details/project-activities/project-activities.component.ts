...
export class ProjectActivitiesComponent implements OnInit {
    ...
    ngOnInit() {
        this.projectService
            .getProjectActivitiesById(this.projectId)
            .subscribe(data => {
    ...
    onFilterChange() {
        ...
        if (this.from != null) {
            this.filteredActivities = this.filteredActivities.filter(
                act => new Date(act.dateTime) >= new Date(this.from)
            );
        }
        if (this.to != null) {
            this.filteredActivities = this.filteredActivities.filter(
